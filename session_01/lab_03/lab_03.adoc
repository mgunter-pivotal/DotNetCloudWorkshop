...
. Name the autoscaling service, place it in your `development` space (which is where the `cf-scale-boot` application is deployed), and bind the autoscaling service to `cf-scale-boot`
+
...
. You will be asked to enable Autoscaling for the `cf-scale-boot` application.  After doing so, configure it with a low threshold so that we can observe the autoscaling.
+
image::../../Common/images/autoscale2.png[]
