...
. Change to the `cf-scale-dotnet` sample application directory:
+
----
$ cd $COURSE_HOME/session_01/lab_03/cf-scale-dotnet
----

. This directory was "published" from Visual Studio. Published .NET applications are ready to run (no build step required.)  So go ahead and push the application:
+
----
$ cf push
----
