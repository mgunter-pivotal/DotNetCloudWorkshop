...
. You will be asked to enable Autoscaling for the `cf-scale-boot` application.  After doing so, configure it with a low threshold so that we can observe the autoscaling. (Configure these settings by clicking the wrench icon)
+
