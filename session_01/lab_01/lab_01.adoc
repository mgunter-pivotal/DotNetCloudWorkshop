...
. UG routes traffic through an http proxy, so you need to set an environment variable instruction the CLI to use that proxy.  From a command line, use this command:
+
----
$ set https_proxy=bcproxy.ugcorp.com
----

. Note that this environment variable only applies to the current command line session. You can alternately add this to the Windows Environment variables so that you don't have to reenter this value every time you use the CLI.
...
. Visit the application in your browser by hitting the route that was generated by the CLI.  You can find the route by typing `cf apps`
+
